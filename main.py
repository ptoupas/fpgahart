...
if __name__ == '__main__':
    ...
    for n,l in partitions.layers.items():
        if l['operation'] == 'Conv':
            ...
            mem_bw = [(0.1,0.9), (0.5,0.5), (0.8,0.2)]
            ...
            for (f, c1, c2, (bw_in, bw_out)) in combinations:
                f_fine, f_fine_elems, f_coarseIn, f_coarseIn_ch, f_coarseOut, f_coarseOut_fil, mem_bw_in, mem_bw_out, dsps_util, max_parallel_muls, bram_util, latency_sec, latency_cycles, throughput_gops, thr_in, thr_out, depth, total_workload, mem_bounded_in, mem_bounded_out = conv.get_design_point(f, c1, c2, conv.mem_words_per_cycle*bw_in, conv.mem_words_per_cycle*bw_out)
                if throughput_gops > max_throughput and (dsps_util < 90. and bram_util < 90.):
                    max_throughput = throughput_gops
                    best = [f_fine, f_fine_elems, f_coarseIn, f_coarseIn_ch, f_coarseOut, f_coarseOut_fil, mem_bw_in, mem_bw_out, dsps_util, max_parallel_muls, bram_util, latency_sec, latency_cycles, throughput_gops, thr_in, thr_out, depth, total_workload, mem_bounded_in, mem_bounded_out]
