...
class BaseLayer():
    ...
    def bram_stream_resource_model(self, depth, width):
        ...
        while rest_depth > 0:
            if rest_depth in list(self.BRAM_CONF_DEPTH.keys()):
                ...
                bram_depth = sorted(list(self.BRAM_CONF_DEPTH.keys()))[
                        bisect.bisect_right(sorted(list(self.BRAM_CONF_DEPTH.keys())), rest_depth) - 1]
                rest_depth = rest_depth - bram_depth
    ...
    def dsp_multiplier_resource_model(self, multiplicand_width, multiplier_width, dsp_type="DSP48E1"):
        return math.ceil((multiplicand_width+multiplier_width)/48)

    def get_dp_performance(self, workload_matrix, ii, muls, adds, layer_fifos_arrays, depth, batch=1, kernel_shape = [], coarse_in=1, coarse_out=1, fine=1, coarse_inout=1):
        ...
        if 'sw_lb_3d' in layer_fifos_arrays.keys():
            ...
            line_buffer_3d_brams = self.bram_stream_resource_model(layer_fifos_arrays['sw_lb_3d'], 16)
            line_buffer_2d_brams = self.bram_stream_resource_model(layer_fifos_arrays['sw_lb_2d'], 16)
            window_buffer_3d_brams = self.bram_stream_resource_model(layer_fifos_arrays['sw_wb_3d'], 16)

            sw_brams = kh*(kw-1) * line_buffer_3d_brams + (kh-1) * line_buffer_2d_brams + kh*kw*(kd-1) * window_buffer_3d_brams
            ...
            if 'acc_fifo' in layer_fifos_arrays.keys():
                fifo_accumulator_brams = self.bram_stream_resource_model(layer_fifos_arrays['acc_fifo'], 30)
                array_accumulator_brams = self.bram_memory_resource_model(layer_fifos_arrays['acc_array'], 30)
            ...
            weights_depth = int((kd*kh*kw * channels * filters) / (fine * coarse_in * coarse_out))
            ...
            bram_raw += sw_brams * coarse_in + \
                       (fifo_accumulator_brams + array_accumulator_brams) * coarse_in * coarse_out + \
                       weights_bram * fine * coarse_in * coarse_out

        if 'elemwise_bc' in layer_fifos_arrays.keys():
            array_elemwise_brams = self.bram_memory_resource_model(layer_fifos_arrays['elemwise_bc'], 30)
            bram_raw += array_elemwise_brams * coarse_inout
        
        if 'fc_array' in layer_fifos_arrays.keys():
            array_fc_brams = self.bram_memory_resource_model(layer_fifos_arrays['fc_array'], 30)
            bram_raw += array_fc_brams * coarse_in * coarse_out

        if 'gap_array' in layer_fifos_arrays.keys():
            array_gap_brams = self.bram_memory_resource_model(layer_fifos_arrays['gap_array'], 30)
        ...
        return latency_sec, latency_cycles, thr_in, thr_out, dsps_util, dsp_raw, bram_util, bram_raw, mem_kb_total
