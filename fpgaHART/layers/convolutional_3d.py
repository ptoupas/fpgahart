...
class Convolutional3DLayer(BaseLayer):
    ...
    def get_design_point(
        ...
        gamma_matrix = (
            self.get_rate_matrix(f_fine)
            * self.get_stream_matrix(f_coarseIn, f_coarseOut)
    ...
    def get_rate_matrix(self, f_fine):
        if self.depthwise:
            ...
            rate_matrix[
                2, 3
            ] = f_fine  # f_fine #*self.groups / math.ceil(1/f_coarseOut)
    ...
    def get_stream_matrix(self, f_coarseIn, f_coarseOut):
        if self.depthwise:
            ...
            # Fork
            stream_matrix[0, 1] = math.ceil(self.channels * f_coarseIn)
            stream_matrix[1, 1] = math.ceil(self.channels * f_coarseIn) * math.ceil(
                self.filters * f_coarseOut
            )

            # Convolution 3D
            stream_matrix[1, 2] = math.ceil(self.channels * f_coarseIn) * math.ceil(
                self.filters * f_coarseOut
            )
    ...
    def get_workload_matrix(self):
        ...
        if self.depthwise:
            ...
            workload_matrix[1, 1] = (
                out_volume * kernel_volume * self.channels * self.filters
            )
