...
class PartitionDescriptor(ModelLayerDescriptor):
    ...
    def create_partitions(self, layers: dict) -> list:
        ...
        if self.model_name == 'x3d_m':
            if not self.se_block:
                layer_type_1 = ['Relu', 'Conv', 'Relu', 'Conv', 'GlobalAveragePool',
                                'Conv', 'Relu', 'Conv', 'Sigmoid', 'Mul', 'Swish', 'Conv', 'Conv', 'Add']
                layer_type_2 = ['Relu', 'Conv', 'Relu', 'Conv', 'GlobalAveragePool',
                                'Conv', 'Relu', 'Conv', 'Sigmoid', 'Mul', 'Swish', 'Conv', 'Add']
                layer_type_3 = ['Relu', 'Conv', 'Relu',
                                'Conv', 'Swish', 'Conv', 'Add']
                layer_type_4 = ['Conv', 'Conv', 'Relu', 'Conv', 'Relu', 'Conv', 'GlobalAveragePool',
                                'Conv', 'Relu', 'Conv', 'Sigmoid', 'Mul', 'Swish', 'Conv', 'Conv', 'Add']
                layer_type_5 = ['Relu', 'Conv', 'Relu',
                                'GlobalAveragePool', 'Gemm', 'Relu', 'Gemm']
                ...
                layer_type_1 = ['Relu', 'Conv', 'Relu', 'Conv',
                                'SqueezeExcitation', 'Swish', 'Conv', 'Conv', 'Add']
                layer_type_2 = ['Relu', 'Conv', 'Relu', 'Conv',
                                'SqueezeExcitation', 'Swish', 'Conv', 'Add']
                layer_type_3 = ['Relu', 'Conv', 'Relu',
                                'Conv', 'Swish', 'Conv', 'Add']
            ...
            layer_type_1 = ['Conv', 'Relu', 'Conv',
                            'Relu', 'Conv', 'Conv', 'Add']
            layer_type_2 = ['Relu', 'Conv', 'Relu',
                            'Conv', 'Relu', 'Conv', 'Add']
    ...
    def update_hw_pe(self, graph: nx.DiGraph, groupping: int = 1) -> None:
        ...
        for i in range(0, len(nodes), groupping):
            node = nodes[i]
            if graph.nodes[node]['hw_type'] not in self.hw_pe:
                self.hw_pe.append(graph.nodes[node]['hw_type'])

    def schedule_ops(self, graph: nx.DiGraph, groupping: int = 1) -> list:
        # for node in graph.nodes():
        #     if graph.in_degree[node] == 0:
        #         first_node = node
        #         break
        ...
        for i in range(0, len(nodes), groupping):
            node = nodes[i]
            hw_op = graph.nodes[node]['hw_type']
    ...
    def update_layer_types(self, graph: nx.DiGraph, plot_types: bool = False) -> None:
        ...
        for node in utils.get_nodes_sorted(graph):
            ...
            _logger.warning(
                f"{node} -> {graph.nodes[node]['hw_type']}")
    ...
    def find_common_layers(self, groupping: int = 1) -> None:
        ...
        for i in range(1, 3):
            ...
            self.schedule_ops(graph=graph, groupping=i)
