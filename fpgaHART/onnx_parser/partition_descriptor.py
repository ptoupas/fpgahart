    ...
    def create_partitions(self, layers):
        ...
        if self.model_name == 'x3d_m':
            if self.breakdown_se:
                ...
                layer_queue = deque(maxlen=14)
                layer_queue_operations = deque(maxlen=14)
                for k in layers.keys():
                    ...
                    if list(layer_queue_operations)[:-7] == layer_type_3:
                        final_layers.append(list(layer_queue)[:-7])
                    if list(layer_queue_operations)[:-12] == layer_type_4 and 'Conv_0' in list(layer_queue)[:-12]:
                        final_layers.append(list(layer_queue)[:-12])
                    if list(layer_queue_operations)[7:] == layer_type_5:
                        final_layers.append(list(layer_queue)[7:])
            else:
