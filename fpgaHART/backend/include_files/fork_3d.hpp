...
#pragma HLS DEPENDENCE variable=local_cache RAW intra true
    ...
    pixel_loop: for (unsigned long pixel_index = 0; pixel_index < batch*height*width*depth*channels; pixel_index++) {
		#pragma HLS PIPELINE II=1 rewind
		k1_loop: for (unsigned char k1 = 0; k1 < kh; k1++) {
			k2_loop: for (unsigned char k2 = 0; k2 < kw; k2++) {
				k3_loop: for (unsigned char k3 = 0; k3 < kd; k3++) {
					coarse_loop: for(unsigned int coarse_index=0;coarse_index < coarse_out; coarse_index++) {
						if(coarse_index == 0) {
...
#pragma HLS INLINE OFF
    ...
    loop_batch: for(unsigned int batch_index=0;batch_index<batch;batch_index++) {
  		loop_rows: for(unsigned int row_index=0;row_index<height;row_index++) {
  			loop_cols: for(unsigned int col_index=0;col_index<width;col_index++) {
  				loop_depth: for(unsigned int depth_index=0;depth_index<depth;depth_index++) {
  					loop_channels: for(unsigned int channel_index=0;channel_index<channels;channel_index++) {
