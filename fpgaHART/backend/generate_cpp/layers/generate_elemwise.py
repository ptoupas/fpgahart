        ...
        with cpp.block(f"for(int coarseIndex=0; coarseIndex<{layer_name_upper}_COARSE; coarseIndex++)"):
            ...
            if broadcasting == 0 and op_type == 'ADD':
                cpp(f"add_3d<\n\
                    ...
                    accum_data_t\n\
                >(in_1[coarseIndex],in_2[coarseIndex],out[coarseIndex]);", newlines=2)
...
def generate_elemwise_hpp(name, config, partition_name):
    ...
    hpp(f"#define {layer_name_upper}_{op_type}_BATCH_SIZE \t{layer_name_upper}_BATCH_SIZE")
    hpp(f"#define {layer_name_upper}_{op_type}_CHANNELS \tDIVIDE({layer_name_upper}_CHANNELS, {layer_name_upper}_COARSE)")
    ...
    hpp(f"#define {layer_name_upper}_{op_type}_WIDTH \t{layer_name_upper}_WIDTH", newlines=2)
