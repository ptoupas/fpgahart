...
def generate_conv_cpp(name, config, partition_name):
    ...
    filters = config['shape_out'][1]
    depth_out = config['shape_out'][2]
    height_out = config['shape_out'][3]
    width_out = config['shape_out'][4]
    kd = config['shape_kernel'][0]
    kh = config['shape_kernel'][1]
    kw = config['shape_kernel'][2]
    pad_d = config['padding'][0]
    pad_h = config['padding'][1]
    pad_w = config['padding'][2]
    stride_d = config['stride'][0]
    stride_h = config['stride'][1]
    stride_w = config['stride'][2]
    ...
    with cpp.block(f"void {layer_name_lower}_layer(\n\
        ...
        cpp(f"#pragma HLS ARRAY_PARTITION variable=weights_{layer_name_lower} complete dim=1")
        cpp(f"#pragma HLS ARRAY_PARTITION variable=weights_{layer_name_lower} complete dim=2")
        ...
        if not depthwise:
            ...
            cpp("#pragma HLS ARRAY_PARTITION variable=accum_out complete dim=0", newlines=2)

        with cpp.block(f"for(int i=0; i<{layer_name_upper}_COARSE_IN; i++)"):
            cpp("#pragma HLS unroll", newlines=2)

            if not pointwise:
                cpp(f"sliding_window_3d<\n\
                ...
                cpp(f"fork_3d<\n\
                    ...
                    {layer_name_lower}_data_t\n\
                >(sw_out[i],fork_out[i]);", newlines=2)
            else:
                cpp(f"fork_3d<\n\
                    {layer_name_upper}_FORK_BATCH_SIZE,\n\
                    {layer_name_upper}_FORK_CHANNELS,\n\
            ...
            with cpp.block(f"for(int j=0; j<{inner_block_loop}; j++)"):
                ...
                if pointwise and not depthwise:
                    ...
                    {layer_name_lower}_data_t,\n\
                    accum_data_t\n\
                >(fork_out[i][j],weights_{layer_name_lower}[i][j],conv_out[i][j]);", newlines=2)
                else:
                    cpp(f"conv_3d<\n\
                    {layer_name_upper}_CONV_BATCH_SIZE,\n\
                    {layer_name_upper}_CONV_CHANNELS,\n\
                ...
                if not depthwise:
                    ...
                    accum_data_t\n\
                >(conv_out[i][j],accum_out[i][j]);", newlines=2)
...
def generate_conv_hpp(name, config, partition_name):
    ...
    hpp(f"#define {layer_name_upper}_SW_CHANNELS \tDIVIDE({layer_name_upper}_CHANNELS, {layer_name_upper}_COARSE_IN)")
    ...
    hpp(f"#define {layer_name_upper}_SW_KERNEL_SIZE_DEPTH \t{layer_name_upper}_KERNEL_SIZE_DEPTH")
    hpp(f"#define {layer_name_upper}_SW_KERNEL_SIZE_HEIGHT \t{layer_name_upper}_KERNEL_SIZE_HEIGHT")
    hpp(f"#define {layer_name_upper}_SW_KERNEL_SIZE_WIDTH \t{layer_name_upper}_KERNEL_SIZE_WIDTH")
    ...
    hpp(f"#define {layer_name_upper}_SW_STRIDE_HEIGHT \t{layer_name_upper}_STRIDE_HEIGHT")
    hpp(f"#define {layer_name_upper}_SW_STRIDE_WIDTH \t{layer_name_upper}_STRIDE_WIDTH", newlines=2)
    ...
    hpp(f"#define {layer_name_upper}_CONV_KERNEL_SIZE_DEPTH \t{layer_name_upper}_KERNEL_SIZE_DEPTH")
    hpp(f"#define {layer_name_upper}_CONV_KERNEL_SIZE_HEIGHT \t{layer_name_upper}_KERNEL_SIZE_HEIGHT")
    hpp(f"#define {layer_name_upper}_CONV_KERNEL_SIZE_WIDTH \t{layer_name_upper}_KERNEL_SIZE_WIDTH")
    ...
    hpp(f"#define {layer_name_upper}_ACCUM_GROUPS \t{layer_name_upper}_GROUPS", newlines=2)

    hpp(f"#define {layer_name_upper}_GLUE_BATCH_SIZE \t{layer_name_upper}_BATCH_SIZE")
    hpp(f"#define {layer_name_upper}_GLUE_CHANNELS \tDIVIDE({layer_name_upper}_CHANNELS, {layer_name_upper}_COARSE_IN)")
